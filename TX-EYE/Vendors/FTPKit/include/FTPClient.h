...
typedef NS_OPTIONS(NSInteger, FTPAccessType) {
    FTPAccessTypeAuto,
    FTPAccessTypeASCII,
    FTPAccessTypeBinary,
};

typedef NS_OPTIONS(NSInteger, FTPConnectionMode) {
    FTPConnectionModePassive,
    FTPConnectionModeActive,
};
...
/**
 The access type to FTP Server.
 FTPAccessTypeAuto(Auto select between ASCII and Binary, Not support now)
 FTPAccessTypeASCII
 FTPAccessTypeBinary
 
 The client recognizes these extensions(data from ftp4j) as textual ones:
 ...
 */
@property (nonatomic, assign) FTPAccessType accessType;

/**
 The connection mode between FTP server and client.
 FTPConnectionModeActive
 FTPConnectionModePassive
 */
@property (nonatomic, assign) FTPConnectionMode connectionMode;

/**
 Specifies the socket idle time in milliseconds 
 that triggers calling the user's callback routine.
 */
@property (nonatomic, assign) int idleTime;

/**
 Specifies the number of bytes to transfer 
 between calls to the user's callback routine.
 */
@property (nonatomic, assign) int cbBytes;
